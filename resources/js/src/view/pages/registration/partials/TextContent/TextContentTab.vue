<template>
  <div>
    <!-- #region title  -->
    <div class="card p-5 pb-0 mb-1">
      <div class="col-xl-12 mx-auto">
        <h3 class="text-center mb-5">Title</h3>
      </div>
      <!-- edit label -->
      <label class="font-weight-bolder">Content: </label>
      <b-form-input
        id="title"
        v-model="baseObjToGen.titleAndDescription.title.text"
      ></b-form-input>
    </div>
    <!-- #endregion title  -->
    <!-- #region benefit text -->
    <div class="card p-5 pb-0 mb-1">
      <div class="col-xl-12 mx-auto">
        <h3 class="text-center mb-5">Benefits</h3>
      </div>
      <!-- enable -->
      <div class="d-flex justify-content-between mb-5 pr-5 align-items-center">
        <label class="font-weight-bolder">Enable: </label>
        <b-form-checkbox
          id="required"
          v-model="baseObjToGen.detailObj.isShow"
          switch
          size="lg"
          style="margin-top: -20px;"
        ></b-form-checkbox>
      </div>
      <!-- header -->
      <label class="font-weight-bolder">Header: </label>
      <b-form-input
        class="mb-5"
        id="title3"
        v-model="baseObjToGen.detailObj.mainHeading"
      ></b-form-input>
      <!-- list benefit text -->
      <div
        v-for="(obj, i) in baseObjToGen.detailObj.detailText"
        :i="i"
        :key="i"
      >
        <label class="font-weight-bolder">Row {{ i + 1 }}:</label>
        <div class="d-flex mb-5">
          <b-form-input
            v-model="baseObjToGen.detailObj.detailText[i].label"
            class="mr-3"
          ></b-form-input>
          <b-form-input
            v-model="baseObjToGen.detailObj.detailText[i].content"
          ></b-form-input>
          <span
            class="svg-icon svg-icon-lg svg-icon-danger pt-2 ml-2 hoverdelete"
            @click="deleteDetailText(i)"
          >
            <inline-svg src="media/svg/icons/Code/Minus.svg" />
          </span>
        </div>
      </div>

      <div
        role="alert"
        class="alert alert-custom alert-light-primary svg-icon-2x p-1 lg-screen"
      >
        <div class="d-flex">
          <span class="svg-icon svg-icon-primary svg-icon-2x pr-2"
            ><svg
              version="1.1"
              viewBox="0 0 24 24"
              height="24px"
              width="24px"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              xmlns="http://www.w3.org/2000/svg"
            >
              <desc xmlns="http://www.w3.org/2000/svg">
                Created with Sketch.
              </desc>
              <defs xmlns="http://www.w3.org/2000/svg"></defs>
              <g
                xmlns="http://www.w3.org/2000/svg"
                id="Stockholm-icons-/-Home-/-Bulb1"
                stroke="none"
                stroke-width="1"
                fill="none"
                fill-rule="evenodd"
              >
                <rect id="bound" x="0" y="0" width="24" height="24"></rect>
                <circle
                  id="Combined-Shape"
                  fill="#000000"
                  opacity="0.3"
                  cx="12"
                  cy="9"
                  r="8"
                ></circle>
                <path
                  d="M14.5297296,11 L9.46184488,11 L11.9758349,17.4645458 L14.5297296,11 Z M10.5679953,19.3624463 L6.53815512,9 L17.4702704,9 L13.3744964,19.3674279 L11.9759405,18.814912 L10.5679953,19.3624463 Z"
                  id="Path-69"
                  fill="#000000"
                  fill-rule="nonzero"
                  opacity="0.3"
                ></path>
                <path
                  d="M10,22 L14,22 L14,22 C14,23.1045695 13.1045695,24 12,24 L12,24 C10.8954305,24 10,23.1045695 10,22 Z"
                  id="Rectangle-72-Copy-2"
                  fill="#000000"
                  opacity="0.3"
                ></path>
                <path
                  d="M9,20 C8.44771525,20 8,19.5522847 8,19 C8,18.4477153 8.44771525,18 9,18 C8.44771525,18 8,17.5522847 8,17 C8,16.4477153 8.44771525,16 9,16 L15,16 C15.5522847,16 16,16.4477153 16,17 C16,17.5522847 15.5522847,18 15,18 C15.5522847,18 16,18.4477153 16,19 C16,19.5522847 15.5522847,20 15,20 C15.5522847,20 16,20.4477153 16,21 C16,21.5522847 15.5522847,22 15,22 L9,22 C8.44771525,22 8,21.5522847 8,21 C8,20.4477153 8.44771525,20 9,20 Z"
                  id="Combined-Shape"
                  fill="#000000"
                ></path>
              </g></svg
          ></span>
        </div>
        <div class="alert-text text-dark">
          <div class="alert-heading" style="font-size: 15px; font-weight: 600">
            Use program details in your registration form
          </div>
          You can use the below variables:<br />
          <ul style="padding-left: 20px">
            <li>commission type: {commission_type}</li>
            <li>commission amount: {commission_amount}</li>
            <li>cookie days: {cookie_days}</li>
          </ul>
        </div>
      </div>

      <!-- button add more benefit text -->
      <div class="d-flex justify-content-center">
        <button
          id="more"
          class="btn btn-primary btn-sm"
          @click="onClickAddBenefitText"
        >
          <span class="svg-icon svg-icon-lg svg-icon-white">
            <inline-svg src="media/svg/icons/Code/Plus.svg" />
          </span>
          More
        </button>
      </div>
    </div>
    <!-- #endregion benefit text -->
    <!-- #region button text -->
    <div class="card p-5 pb-0 mb-1">
      <div class="col-xl-12 mx-auto">
        <h3 class="text-center mb-5">Button</h3>
      </div>

      <label class="font-weight-bolder">Button content: </label>
      <b-form-input
        id="title1"
        v-model="baseObjToGen.button.submittext"
      ></b-form-input>
    </div>
    <!-- #endregion button text -->
    <!-- #region aside -->
    <div v-if="this.baseObjToGen.name != 'basicObj'" class="card pt-10 mb-1">
      <div class="col-xl-12 mx-auto">
        <h3 class="text-center">Aside Content</h3>
      </div>
      <editor
        ref="editor"
        v-model="textData"
        :plugins="myPlugins"
        :toolbar="myToolbar"
        :api-key="editorKey"
        :init="myInit"
      />
    </div>
    <!-- #endregion aside -->
  </div>
</template>

<script>
import Editor from "@tinymce/tinymce-vue";
import LoadingSubmitButton from "@/view/content/LoadingSubmitButton.vue";
import ApiService from "@/core/services/api.service";
import { mapGetters } from "vuex";
import {
  BFormSelect,
  BFormInput,
  BSpinner,
  BButton,
  BFormCheckbox,
  BFormTextarea,
  BOverlay,
  BFormGroup,
  BInputGroup,
} from "bootstrap-vue";
import FontPicker from "font-picker-vue";
export default {
  components: {
    BFormSelect,
    BFormInput,
    BSpinner,
    BButton,
    BFormCheckbox,
    BFormTextarea,
    BOverlay,
    BFormGroup,
    LoadingSubmitButton,
    FontPicker,
    BInputGroup,
    editor: Editor,
  },
  data() {
    return {
      submitLoading: false,
      myPlugins: [
        "advlist autolink lists link image charmap print preview anchor",
        "searchreplace visualblocks code fullscreen",
        "insertdatetime media table paste code help wordcount emoticons",
      ],
      myToolbar:
        "undo redo | formatselect | bold italic forecolor backcolor | code \
            alignleft aligncenter alignright alignjustify | image |charmap emoticons\
            bullist numlist outdent indent | removeformat",
      myInit: {
        convert_urls: true,
        height: 400,
        automatic_uploads: true,
        relative_urls: true,
        menubar: true,
        selector: "textarea#local-upload",
        images_upload_handler: function (blobInfo, success, failure) {
          var image;
          image = new FormData();
          image.append("image", blobInfo.blob(), blobInfo.filename());
          ApiService.post("categorys/image-editor", image)
            .then(({ data }) => {
              success(data.data.location);
            })
            .catch(({ response }) => {
              failure("Something went wrong: " + response);
              return;
            });
        },
      },
    };
  },
  methods: {
    onClickAddBenefitText() {
      this.baseObjToGen.detailObj.detailText.push({ label: "", content: "" });
    },
    deleteDetailText(i) {
      this.baseObjToGen.detailObj.detailText.splice(i, 1);
    },
  },
  computed: {
    ...mapGetters(["baseObjToGen"]),
    id() {
      return this.$route.params.id;
    },
    textData: {
      get: function () {
        return this.baseObjToGen.aside.textContent || "";
      },
      set: function (newValue) {
        this.baseObjToGen.aside.textContent = newValue;
      },
    },
    editorKey() {
      return process.env.MIX_SITE_KEY_EDITOR;
    },
  },
};
</script>
<style scoped>
.card {
  box-shadow: none !important;
  border-radius: unset !important;
}
.card-btn {
  position: fixed;
  top: 0px;
  right: 0;
  left: 0;
  z-index: 97;
}
@media screen and (max-width: 992px){
    .lg-screen{
        display: none;
    }
  }
</style>
<style lang="scss">
.font-picker {
  width: 100% !important;
  box-shadow: none !important;
  border: 1px solid #e4e6ef !important;
  border-radius: 0.42rem !important;
  .dropdown-button {
    background-color: #fff !important;
  }
}
.hoverdelete {
  cursor: pointer;
}
</style>
